...
@app.post("/items/{item_id}/images", tags=["Image Management"])
async def add_item_images(
    ...
    for image in images:
        if image.filename:
            ...
            unique_filename = f"{current_user.username}_{item.name}_{_uuid.uuid4().hex[:8]}.{file_extension}"
            image_path = f"{vendor_folder}/{unique_filename}"
            ...
            new_image_paths.append(f"/uploads/{current_user.username}/{unique_filename}")
    ...
    return {"message": f"Added {len(new_image_paths)} images", "total_images": len(all_images)}
...
async def remove_item_image(
    ...
    """Remove specific image from item by index"""
    if not current_user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Not authorized")
    ...
    return {"message": "Image removed", "remaining_images": len(images)}
...
async def create_items(
    ...
    """Create a new item with optional multiple images"""
    if not current_user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="You're not authorized!",
    ...
    if images:
        for image in images:
            if image.filename:
                ...
                # Create unique filename with vendor prefix
                file_extension = image.filename.split(".")[-1]
                unique_filename = f"{current_user.username}_{name}_{_uuid.uuid4().hex[:8]}.{file_extension}"
                image_path = f"{vendor_folder}/{unique_filename}"
                ...
                image_paths.append(f"/uploads/{current_user.username}/{unique_filename}")
    
    # Create ItemCreate object
