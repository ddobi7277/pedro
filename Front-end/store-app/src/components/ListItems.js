...
function ListItems({ items, username }) {
  ...
  // Función para determinar qué columnas ocultar en diferentes dispositivos
  const shouldHideColumn = (column) => {
    // En móvil: Solo mostrar 6 columnas esenciales: name, status, inversion, revenue, detalles, actions
    const hideOnMobile = ['firstImage', 'sku', 'cant', 'precio', 'costo', 'sales'];
  ...
  const columns = [
    {
      field: 'name',
      headerName: isMobile ? 'Producto' : 'Product',
    ...
    {
      field: 'firstImage',
      headerName: isMobile ? 'Foto' : 'Photo',
    ...
    {
      field: 'status',
      headerName: isMobile ? 'Estado' : 'Status',
      width: getColumnWidth(120, 100, 120, 140, 'status'),
      editable: false,
      renderCell: (params) => {
    ...
    {
      field: 'cant',
      headerName: isMobile ? 'Stock' : 'Stock',
  ...
  return (
    <div>
      ...
      <div>
        ...
        {/* Detalles expandibles - Solo para mobile/tablet */}
        {(isMobile || isTablet) && Array.from(expandedRows).map(rowId => {
          const row = orows.find(r => r.id === rowId);
          if (!row) return null;

          const isInEditMode = rowModesModel[rowId]?.mode === GridRowModes.Edit || cellEditingRows.has(rowId);
          const changes = editingChanges[rowId] || {};
          const currentRow = { ...row, ...changes };

          // Calcular valores dinámicos
          const costoUSD = parseFloat(currentRow.cost || row.cost) || 0;
          const costoMN = costoUSD * tasaCambio;
          const precioMN = parseFloat(currentRow.price_MN || currentRow.price || row.price) || 0;
          const precioUSD = parseFloat(currentRow.price_USD || row.price_USD) || 0;
          const cantidad = parseFloat(currentRow.cant || row.cant) || 0;
          const inversionTotal = costoUSD * cantidad;
          const gananciaTotal = (precioUSD * cantidad) - inversionTotal;
            ...
            >
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 2, mb: 2 }}>
                ...
                <Box>
                  ...
                  <Box sx={{ mt: 0.5 }}>
                    {row.firstImage ? (
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <img
                          src={getImageUrl(row.firstImage)}
                          alt="Product"
                          style={{
                            width: 40,
                            height: 40,
                          ...
                          onClick={() => {
                            if (row.images && row.images.length > 0) {
                              openImageModal(row.images, 0, row);
                            }
                          }}
                        />
                        {row.images && row.images.length > 1 && (
                          <Typography variant="caption" color="text.secondary">
                            +{row.images.length - 1} más
                          </Typography>
                        )}
                      </Box>
                    ) : (
                ...
                <Box>
                  ...
                  {isInEditMode ? (
                    <TextField
                      size="small"
                      type="number"
                      defaultValue={currentRow.cant || 0}
                      onChange={(e) => {
                        setEditingChanges(prev => ({
                          ...prev,
                          [rowId]: {
                ...
                <Box>
                  ...
                  <Typography variant="body2">
                    0 {/* TODO: Conectar con tabla sales */}
                  </Typography>
                </Box>
                ...
                {isInEditMode ? (
                  ...
                  <Box
                    sx={{
                      ...
                      border: '1px solid #e0e0e0',
                      borderRadius: 1,
                      cursor: isInEditMode ? 'default' : 'pointer',
                      '&:hover': !isInEditMode ? {
                        backgroundColor: '#f5f5f5',
                        borderColor: '#1976d2'
                      } : {}
        ...
        {/* Detalles expandibles - Solo para desktop (versión simple) */}
        {!isMobile && !isTablet && Array.from(expandedRows).map(rowId => {
          const row = orows.find(r => r.id === rowId);
          if (!row) return null;
