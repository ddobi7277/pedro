...
<div>
            {item &&
            <div>
            <label>Preferencia de Moneda: <select onChange={(e)=> {setCurrency(e.target.value)}}>{['USD','MN'].map((option)=> (
                        <option key={option} value={option}>{option}</option>
                    ))}</select></label><p/>
            <label> ID:{item.name}</label><p/>
            <label>Nombre: <input value={name} onChange={(e) => {setName(e.target.value)}} /></label><p/>
            <label>Costo: <input disabled={currency=== 'MN'} value={currency==='MN'?Math.round(costMN/tasaCambio*100)/100:costUSD} onChange={(e) => {setCost(e.target.value)}} />USD -- <input disabled={currency=== 'USD'} value={currency==='USD'?costUSD*tasaCambio:costMN} onChange={(e) => {setCostMN(e.target.value)}} />MN </label><p/>
            <label>Precio: <input value={price} onChange={(e) => {setPrice(e.target.value)}} /></label><p/>
            <label>Cantidad: <input value={cant} onChange={(e) => {setCant(e.target.value)}} /></label><p/>
            ...
            {
            show&&<button onBlur={() => {setEditCat(false);setNewCat(false)}} onClick={() => {handleNewCategory()}}>+</button>}{show&&<button onBlur={() => {setEditCat(false);setNewCat(false)}} onClick={() => {handleEditCat()}}>Editar Categoria</button>}{show&&<button onClick={handleDeleteCat}>Eliminar</button>}
            {errorC && <p style={{color:'red'}}>Ya esta categoria existe para ti</p>}
            ...
            <label>Pais donde lo compro: <select onFocus={() => {setShow(false)}} onChange={(e)=> {setCountry(e.target.value)}}>{['Cuba','Afuera'].map((option)=> (
                        <option key={option} value={option}>{option}</option>
                    ))}</select></label><p/> 
            <button onClick={() => {handleSubmit()}}>Submit</button>
            <button onClick={() => {navigate('/dashboard')}}>Atras</button>
            </div>
            }
